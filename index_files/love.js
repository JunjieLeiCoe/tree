(function(window){
    ...
    Tree.prototype = {
        ...
        jump: function() {
            ...
            // Calculate days for reference only
            var together = new Date(config.date);
            var now = new Date();
            var days = Math.floor((now.getTime() - together.getTime()) / (24 * 60 * 60 * 1000));
            
            // Only add new hearts when there are fewer than 3 visible hearts
            // This mimics the original behavior while maintaining day-based hearts
            if ((blooms.length && blooms.length < 3) || !blooms.length) {
                ...
                // Add only 1-2 hearts at a time like the original code
            ...
            // Randomly add stars - low frequency for subtle effect
            if (random(0, 100) < 0.8 && (!this.stars || this.stars.length < 15)) {
                var x = random(this.width * 0.3, this.width * 0.7);
                var y = random(this.height * 0.1, this.height * 0.3);
                this.addStar(x, y);
            }
        },
        ...
        addStar: function(x, y) {
            ...
            this.stars.push(new Star(this, new Point(x, y)));
        },
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        ...
        // Base color for the heart
        this.baseColor = color || 'rgb(255,' + random(150, 255) + ',' + random(150, 255) + ')';
        
        // Generate a slightly different shade for gradient
        var r = 255;
        var g = random(100, 200);
        var b = random(100, 200);
        this.gradientColor = 'rgb(' + r + ',' + g + ',' + b + ')';
    ...
    Bloom.prototype = {
        ...
        draw: function() {
            ...
            // Create radial gradient for heart
            var gradient = ctx.createRadialGradient(0, 0, 0, 0, 0, 15);
            gradient.addColorStop(0, s.baseColor);
    ...
    Star = function(tree, point, color) {
        this.tree = tree;
        this.point = point;
        // Use golden/yellow colors for stars
        this.color = color || 'rgb(' + random(220, 255) + ',' + random(180, 220) + ',' + random(0, 60) + ')';
        this.alpha = random(0.6, 1);
        this.angle = random(0, 360);
        this.scale = random(0.3, 0.6);
        this.speed = random(1, 2);
        this.rotation = random(0.01, 0.03);
        this.place = new Point(random(0, tree.width), tree.height + 10);
    ...
    Star.prototype = {
        draw: function() {
            ...
            // Add highlight to create twinkling effect
            ctx.globalAlpha = Math.abs(Math.sin(s.angle * 3)) * 0.3;
            ctx.fillStyle = "rgba(255, 255, 255, 0.8)";
            ctx.beginPath();
            
            // Smaller star for the highlight
            ctx.moveTo(0, -outerRadius/2);
            for (var i = 0; i < spikes * 2; i++) {
                var radius = i % 2 === 0 ? innerRadius/2 : outerRadius/2;
                var angle = Math.PI * i / spikes;
                ctx.lineTo(Math.sin(angle) * radius, -Math.cos(angle) * radius);
            }
        ...
        fall: function() {
            ...
            if (s.point.y > s.tree.height || s.alpha <= 0) {
                s.tree.removeStar(s);
                return;
            }
            ...
            // Simple falling with slight side movement
            s.point.y += s.speed;
            s.point.x += Math.sin(s.angle) * 0.5;
            s.angle += s.rotation;
            
            // Twinkle effect
            s.alpha += s.twinkleSpeed * s.twinkleSign;
            if (s.alpha >= 1) {
                s.alpha = 1;
                s.twinkleSign = -1;
            } else if (s.alpha <= 0.6) {
            ...
            if (s.point.y > s.tree.height * 0.7) {
                s.alpha -= 0.02; // Faster fade out
            }
        }
    }
